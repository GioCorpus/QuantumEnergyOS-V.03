#!/bin/bash
# ──────────────────────────────────────────────────────────────────────────────
# install-fedora.sh — QuantumEnergyOS Installation Script for Fedora
# and Fedora-based distributions (Rocky Linux, AlmaLinux, CentOS Stream, etc.)
#
# Usage: chmod +x install-fedora.sh && ./install-fedora.sh
# ──────────────────────────────────────────────────────────────────────────────
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     QuantumEnergyOS - Fedora Installation Script             ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Detect Fedora-based distribution
echo -e "${BLUE}[1/6] Detecting distribution...${NC}"
if [[ -f /etc/fedora-release ]]; then
    source /etc/os-release
    echo "Found: $NAME ($VERSION_ID)"
elif [[ -f /etc/rocky-release ]]; then
    source /etc/rocky-release
    echo "Found: Rocky Linux ($VERSION)"
elif [[ -f /etc/almalinux-release ]]; then
    source /etc/almalinux-release
    echo "Found: AlmaLinux ($VERSION)"
elif [[ -f /etc/centos-release ]]; then
    source /etc/centos-release
    echo "Found: CentOS ($VERSION)"
else
    echo -e "${YELLOW}Warning: Could not detect specific distribution. Assuming Fedora.${NC}"
fi
...
sudo dnf install -y "${DEPS_BASE[@]}" "${DEPS_EXTRA[@]}"
...
if [[ $(echo "$PYTHON_VERSION < 3.11" | bc) -eq 1 ]]; then
    ...
    echo "    You can install Python 3.11 with: sudo dnf install python3.11${NC}"
fi
